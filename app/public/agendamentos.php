...
include('./components/topbar/topbar.php');
...
while ($client_data = mysqli_fetch_assoc($result)) {
    ...
    if (!empty($data) && $data !== '0000-00-00') {
        $agendamentosPorDia[$data][] = [
            ...
            'data_id' => $client_data['agendamento_data_id'],
            'data' => $client_data['data'],
            'cliente' => $client_data['nome_cliente'],
            'pet' => $client_data['nome_pet'],
            'peso' => $client_data['peso'],
            ...
            'servico' => $client_data['servicos'],
            'valor' => $client_data['valores_servicos'],
            'hora' => $client_data['hora'] ?? null,
            'obs_cliente' => $client_data['observacoes_cliente'],
            'observacoes_pos_banho' => $client_data['observacoes_pos_banho'],
            'retirada_cliente' => $client_data['retirada_cliente'],
        ];
    }
}
...
foreach ($agendamentosPorDia as $data => $agendamentos) {
    ...
    foreach ($agendamentos as $a) {
        ...
        echo "<p><strong>Obs. Banho e tosa:</strong> {$a['observacoes_pos_banho']}</p>";
        echo "<p><strong>Retirada:</strong> {$a['retirada_cliente']}</p>";
        ...
        echo "<select class='select_btn' name='novo_status' onchange='this.form.submit()'>";
...
<div style="text-align:center; margin: 20px;">
    <?php if ($pagina > 1): ?>
        <a href="?pagina=<?= $pagina - 1 ?>" style="margin-right:10px;">⬅ Anterior</a>
    <?php endif; ?>

    <?php 
    // Mostrar links de páginas
    for ($i = 1; $i <= $paginas; $i++): 
        if ($i == $pagina): ?>
            <strong>[<?= $i ?>]</strong>
        <?php else: ?>
    ...
    <?php if ($pagina < $paginas): ?>
        <a href="?pagina=<?= $pagina + 1 ?>" style="margin-left:10px;">Próxima ➡</a>
    <?php endif; ?>
</div>
